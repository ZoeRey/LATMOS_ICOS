...
#%%moyenne sur 1h ou de 19h44 à 22h42
t1=1184
t2=1363
t3=["19:44-22:42 UT"]    
...
#%%Calcul constante en échelle log
...
#%% test pour géométrie (pas besoin si géométrie déterminée)
...
#%%suite test géométrie (pas besoin si géométrie déterminée)
c=[]
for i in range(0,60):
    #print(altitude[i])
...
alpha532_aer,AOD532,trans532_aer=outils_OHP.calc_alpha_AOD_trans(beta532_aer,LRp,altitude[:izr])
...
plt.figure()
plt.title("ICOS 532// "+t3[0])
plt.plot(beta532_aer[iav:1400],altitude[iav:1400])
#plt.grid()
...
#%%zoom pic et calcul AOD pic et AOD tot
...
plt.figure()
#plt.title("ICOS 532// "+t3[0])
plt.plot(beta532_aer[i_pic_begin:i_pic_end],altitude[i_pic_begin:i_pic_end])
plt.grid()
plt.xlabel("beta aer 532// (km⁻¹.sr⁻¹)")
plt.ylabel("altitude (km)")
plt.legend()
...
AOD_tot=np.trapz(alpha532_aer[1:1388],x=altitude[1:1388])
...
for i in range(len(beta532_aer)):
    R532_ICOS.append(1.+beta532_aer[i]/rayleigh532['beta_m'][i])
